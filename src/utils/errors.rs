...
use std::fmt;
use ethers::signers::WalletError; // <-- Import WalletError
use ethers::providers::ProviderError; // <-- Import ProviderError
...
pub enum AppError {
    #[error("Smart contract interaction error: {0}")]
...
// ===== FIX #1: ADD THE 'FROM' IMPLEMENTATIONS FOR ETHERS ERRORS =====

// Conversion from ethers-rs WalletError
impl From<WalletError> for AppError {
    fn from(err: WalletError) -> Self {
        AppError::InternalServerError(format!("Wallet configuration error: {}", err))
    }
}

// Conversion from ethers-rs ProviderError
impl From<ProviderError> for AppError {
    fn from(err: ProviderError) -> Self {
        AppError::ContractError(format!("Blockchain provider RPC error: {}", err))
    }
}
impl AppError {
    pub fn error_code(&self) -> &'static str {
        match self {
            // ===== ADDED THIS LINE =====
            AppError::ContractError(_) => "BC_008",

            // --- Existing Codes Below ---
    ...
    pub fn status_code(&self) -> StatusCode {
        match self {
            // ===== ADDED THIS LINE =====
            AppError::ContractError(_) => StatusCode::UNPROCESSABLE_ENTITY,
            ...
            AppError::RateLimitError(_) | AppError::TooManyRequests => StatusCode::TOO_MANY_REQUESTS,
