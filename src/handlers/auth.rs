...
// tokenization-backend/src/handlers/auth.rs
// UPDATED WITH EMAIL VERIFICATION
...
use crate::utils::auth::Claims;
// ✨ ADD THIS IMPORT
use crate::handlers::email_verification::send_verification_email;
...
// ✨ ADD THIS NEW RESPONSE STRUCT FOR REGISTRATION
#[derive(Debug, Serialize)]
...
pub async fn register(
    ...
    let new_user = User {
        ...
        email_verified: Some(false), // ✨ Set to false initially
        phone_verified: Some(false),
        two_factor_enabled: Some(false),
        two_factor_secret: None,
        last_login: None,
        ...
        reset_token: None,
        reset_token_expires: None,
        verification_token: None, // ✨ Will be set by send_verification_email
    ...
    info!("User registration successful for: {} (verification email sent)", user.email);

    // ✨ CHANGED: Return registration response instead of auth response
...
pub async fn login(
    ...
    if !user.email_verified.unwrap_or(false) {
        warn!("Login attempt with unverified email: {}", payload.email);
