...
// // ===== FIX #1: ADD TryFrom<String> IMPLEMENTATIONS =====
// // This teaches sqlx how to convert a string from the DB into your enums.

// impl TryFrom<String> for DocumentType {
//     type Error = AppError;
//     fn try_from(value: String) -> Result<Self, Self::Error> {
//         Self::from_str(&value).map_err(|_| AppError::InternalServerError("Failed to parse DocumentType from database".to_string()))
//     }
// }

// impl TryFrom<String> for DocumentVerificationStatus {
//     type Error = AppError;
//     fn try_from(value: String) -> Result<Self, Self::Error> {
//         Self::from_str(&value).map_err(|_| AppError::InternalServerError("Failed to parse DocumentVerificationStatus from database".to_string()))
//     }
// }
...
//     if request.user_id != user.id && !matches!(user.role, UserRole::Admin | UserRole::ComplianceOfficer) {
//         return Err(AppError::Forbidden("You can only initiate KYC for yourself".to_string()));
//     }
...
//         return Err(AppError::Forbidden("You can only view your own KYC status".to_string()));
//     }
...
//             uploaded: uploaded_documents.iter().any(|d| matches!(d.document_type, DocumentType::Passport | DocumentType::NationalId)),
//         },
//         DocumentRequirement {
//             document_type: DocumentType::UtilityBill,
//             required: true,
//             description: "Proof of address (utility bill or bank statement)".to_string(),
//             uploaded: uploaded_documents.iter().any(|d| matches!(d.document_type, DocumentType::UtilityBill | DocumentType::BankStatement)),
//         },
//     ];
...
// ===== FIX #2: ADD FromRow AND IMPLEMENT TryFrom =====
#[derive(Debug, Serialize, Deserialize, sqlx::FromRow)]
pub struct DocumentSummary {
    pub id: Uuid,
    #[sqlx(try_from = "String")]
